...
function FormLogin() {
  ...
  //   clientSecret: import.meta.env.VITE_GG_CLIENT_SECRET,
  //   redirect_uri: import.meta.env.VITE_GG_REDIRECT_URI,
  //   scope: "email profile",
  //   onSucess: (response) => {
  ...
  return (
    <Form noValidate validated={validated} onSubmit={handleSubmit}>
      ...
      <Row className="mb-3">
        <Form.Group as={Col} md="12" controlId="validationPassword">
          ...
          <InputGroup hasValidation>
            ...
            >
              <Button
                className="btn p-0 border-0 bg-white"
                type="button"
      ...
      <Row className="mb-4">
        <div className="text-start mb-4">
          <span className="text-secondary text-sm">
            ...
            <a className="ps-1" href="https://policies.google.com/privacy">
              Privacy Policy
            </a>{" "}
            và
            <a className="ps-1" href="https://policies.google.com/terms">
              Terms of Service
            </a>{" "}
            apply.
          </span>
        ...
        <Form.Group className="mb-0 d-flex justify-content-between" noValidate>
          <Form.Check
            ...
            noValidate
          />
          <Link className="float-end" to={"/lazi-store/quen-mat-khau"}>
            Quên mật khẩu
          </Link>
        </Form.Group>
        <div className="text-center text-sm">
          {success && (
            <span className={"text-success"}>Đăng nhập thành công</span>
          )}
          {unSuccess && (
            <span className={"text-danger"}>
              Tài khoản chưa được đăng ký, vui lòng thử lại
            </span>
          )}
        </div>
      </Row>
      ...
      <div className="d-flex justify-content-center mb-3">
        <Button
          type="submit"
          className={`w-50 rounded-3 fw-medium ${loading && "disabled"}`}
        >
          {loading ? (
            <span>
              Đang đăng nhập <Loading width={20} height={20} />{" "}
            </span>
          ) : (
            <span>Đăng nhập</span>
          )}
      ...
      <Row className="mb-3">
        <Col className="d-flex justify-content-center">
          <Button
            className="text-start w-75 fw-bold btn-light rounded-2"
            type="button"
            onClick={handleLoginFaceBook}
          >
            <i className="fab fa-facebook-square"></i> Đăng nhập bằng Facebook
          </Button>
        </Col>
      </Row>
    </Form>
