...
function Header() {
  ...
  const handleDropdownClick = () => {
    setShowDropdown(!showDropdown); // Đảo ngược giá trị của showDropdown khi click
  };
  const handleLogout = () => {
  ...
  const loginSettings = [
    ...
    { id: 2, name: "Đơn hàng", link: "/don-hang" },
              ...
              <ul className="navbar-nav ms-auto">
                ...
                {hasLogin ? (
                  <li className="nav-item dropdown me-3">
                   <div className="nav-link dropdown-toggle" onClick={handleDropdownClick}>
        {userInfo.name}
      </div>
                   
      <div
        className={`dropdown-menu mt-3 shadow-sm w-25 ${showDropdown ? 'show' : ''}`} // Thêm class 'show' nếu showDropdown là true
        style={{ opacity: showDropdown ? 1 : 0, visibility: showDropdown ? 'visible' : 'hidden' }} // Thay đổi opacity và visibility
